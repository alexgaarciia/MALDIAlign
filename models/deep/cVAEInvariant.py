...
class InvariantCVAE_Bernoulli(nn.Module):
    ...
    The encoder learns a latent representation z from x that is invariant to the
    conditioning variable c, while the decoder models p(x | z, c). This setup is
    commonly used to encourage domain-invariant latent spaces.
    """
    ...
    def forward(self, x, c):
        """
        Forward pass of the invariant CVAE.
        ...
        x : torch.Tensor
            Input data.
        c : torch.Tensor
            Conditioning variable (used only by the decoder).
    ...
    def elbo_loss(self, x, mu, logvar, z, c, beta=1.0):
        """
        Computes the ELBO loss for the invariant CVAE.
...
class InvariantCVAE_Bernoulli_Extended(InvariantCVAE_Bernoulli):
    """
    Extended invariant CVAE including optimizer setup, training loop,
    KL annealing, early stopping, and loss tracking.
    """

    def __init__(self, input_dim, latent_dim, cond_dim, lr=1e-4, epochs=100, patience=20, annealing_epochs=50):
        """
        Parameters
        ----------
        input_dim : int
