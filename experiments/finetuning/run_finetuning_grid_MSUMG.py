...
for n_prev in grid_prev:
    for n_new in grid_new:
        ...
        for fold_i, (_, idx_f) in enumerate(skf.split(X_test, y_test_enc)):
            ...
            Z_dec_f = Z_test_dec[idx_f]
            Z_partial_f = Z_test_partial[idx_f]
            ...
            loader_dec_f = make_loader(Z_dec_f, y_te_f_enc)
            loader_partial_f = make_loader(Z_partial_f,y_te_f_enc)
 
            # Baseline RF/MLP in original space
            metrics_orig = metrics_report(X_te_f, y_te_f, baseline_rf_original, "MS-UMG", np.unique(TARGET_SPECIES))
            metrics_mlp_orig = metrics_report_mlp(loader_orig_f, baseline_mlp_original, "MS-UMG", device=device, class_names=le.classes_)
 
            # Baseline latent (zero-shot)
            metrics_lat = metrics_report(Z_zero_f, y_te_f, baseline_rf_latent, "MS-UMG", np.unique(TARGET_SPECIES))
            metrics_mlp_lat = metrics_report_mlp(loader_zero_f, baseline_mlp_latent, "MS-UMG", device=device, class_names=le.classes_)
 
            # Few-shot on target
            metrics_few = metrics_report(X_te_f, y_te_f, rf_few, "MS-UMG", np.unique(TARGET_SPECIES))
            metrics_mlp_few_orig = metrics_report_mlp(loader_orig_f, mlp_few_orig, "MS-UMG", device=device, class_names=le.classes_)
 
            # Finetuning full
            metrics_ft_full = metrics_report(Z_full_f, y_te_f, baseline_rf_latent, "MS-UMG", np.unique(TARGET_SPECIES))
            metrics_ft_full_mlp = metrics_report_mlp(loader_full_f, baseline_mlp_latent, "MS-UMG", device=device, class_names=le.classes_)
 
            # Finetuning freeze_priors
            metrics_ft_freeze = metrics_report(Z_freeze_f, y_te_f, baseline_rf_latent, "MS-UMG", np.unique(TARGET_SPECIES))
            metrics_ft_freeze_mlp = metrics_report_mlp(loader_freeze_f, baseline_mlp_latent, "MS-UMG", device=device, class_names=le.classes_)
 
            # Finetuning decoder_only
            metrics_ft_dec = metrics_report(Z_dec_f, y_te_f, baseline_rf_latent, "MS-UMG", np.unique(TARGET_SPECIES))
            metrics_ft_dec_mlp = metrics_report_mlp(loader_dec_f, baseline_mlp_latent, "MS-UMG", device=device, class_names=le.classes_)
 
            # Finetuning partial_encoder
            metrics_ft_partial = metrics_report(Z_partial_f, y_te_f, baseline_rf_latent, "MS-UMG", np.unique(TARGET_SPECIES))
            metrics_ft_partial_mlp = metrics_report_mlp(loader_partial_f, baseline_mlp_latent, "MS-UMG", device=device, class_names=le.classes_)
