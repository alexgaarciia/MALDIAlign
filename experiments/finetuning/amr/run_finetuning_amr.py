...
def run_finetuning_amr(
    ...
    if len(X_ft) == 0:
        ...
        vae = MultiVAE_Bernoulli_SpeciesPrior_AMR_Head_Extended(
            input_dim=data_norm.shape[1], latent_dim=latent_dim,
            num_domains=n_pretrained_domains, n_species=n_species_pretrained,
            n_antibiotics=len(keep_names), lambda_amr=lambda_amr_eff,
    ...
    ####################
    pos_weights = []
    for j in range(amr_ft.shape[1]):
        y_j = amr_ft[:, j]
        valid = ~np.isnan(y_j)
        n_pos = (y_j[valid] == 1).sum()
        n_neg = (y_j[valid] == 0).sum()
    ...
    print(f"Calculated pos_weight per antibiotic: {pos_weight_tensor.numpy().round(2)}")
    ...
    if finetuning_mode == "amr_head":
        ...
        raise ValueError(
            ...
            "Options: amr_head, enc_dec, enc_dec_amr"
        )
    ...
    vae.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(vae.optimizer, mode="max", factor=0.5, patience=5, min_lr=1e-6)
...
def _count_species(model_path):
    """
    Infer n_species from the species-prior parameters in a saved state dict.

    The SpeciesPrior stores per-species mu/logvar tensors of shape
    (n_species, latent_dim).  We look for the first 2-D tensor whose
    key contains 'prior' to read n_species.
    Falls back to len(TARGET_SPECIES) if nothing matches.
    """
    state = torch.load(model_path, map_location="cpu")
    for k, v in state.items():
        if "prior" in k and v.ndim == 2:
            return v.shape[0]
    for k, v in state.items():
        if "species" in k and v.ndim >= 1:
            return v.shape[0]
    print(
        "WARNING: could not infer n_species from state dict; "
        f"falling back to len(TARGET_SPECIES)={len(TARGET_SPECIES)}"
    )
    return len(TARGET_SPECIES)
