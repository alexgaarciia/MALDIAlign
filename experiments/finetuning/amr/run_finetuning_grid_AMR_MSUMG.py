...
TARGET_DOMAINS = ["MS-UMG_2020", "MS-UMG_2021"]
DOMAIN_FILTERS = {
    "MS-UMG_2020": ("MS-UMG", "2020"),
    "MS-UMG_2021": ("MS-UMG", "2021"),
}
SPLITS_PATH_PER_DOMAIN = {
    "MS-UMG_2020": Path("/export/usuarios01/agnavarr/MALDIAlign/experiments/finetuning/amr/output_data/splits_Klebsiella_Pneumoniae_MSUMG_2020_20260506_231850"),
    "MS-UMG_2021": Path("/export/usuarios01/agnavarr/MALDIAlign/experiments/finetuning/amr/output_data/splits_Klebsiella_Pneumoniae_MSUMG_2021_20260506_231850"),
}
SPECIES = "Klebsiella_Pneumoniae"

PRETRAINED_MODEL_PATH = Path("/export/usuarios01/agnavarr/MALDIAlign/experiments/results/vae_multidecoder_prior_multiamr_per_year/20260506_230712/model.pth")
OUTPUT_PATH = Path("/export/usuarios_ml4ds/agnavarr/MALDIAlign/finetuning_amr_kpneu_per_year")
...
LATENT_DIM = 128
LAMBDA_AMR = 100
NUM_DOMAINS = 12  # center x year domains from per-year model
...
# Load each MS-UMG year separately
target_datasets = {}
for target_dom in TARGET_DOMAINS:
    hospital, year = DOMAIN_FILTERS[target_dom]
    dom_mask = (
        (meta_raw["hospital"] == hospital) &
        (meta_raw["year"].astype(str) == year)
    ).values

    if "agar" in meta_raw.columns:
        chrom = (meta_raw["agar"] == "chrom").values
        n_removed = int((dom_mask & chrom).sum())
        dom_mask = dom_mask & ~chrom
        if n_removed > 0:
            print(f"  {target_dom}: removed {n_removed} chrom-agar")

    species_mask = (labels_raw == SPECIES)
    local_mask = dom_mask & species_mask

    data_t   = data_norm[local_mask]
    amr_t    = amr_aligned[local_mask]
    labels_t = labels_raw[local_mask]
    meta_t   = meta_raw.loc[local_mask].reset_index(drop=True)

    target_datasets[target_dom] = {
        "data": data_t, "amr": amr_t, "labels": labels_t, "meta": meta_t,
    }

    print(f"\n{target_dom} ({hospital} {year}): {len(data_t)} samples")
...
for TARGET_DOMAIN in TARGET_DOMAINS:
    SPLITS_PATH = SPLITS_PATH_PER_DOMAIN[TARGET_DOMAIN]
    td = target_datasets[TARGET_DOMAIN]
    ...
    for i_part in range(N_PARTITIONS):
        ...
        for n_prev in GRID_PREV:
            for n_new in GRID_NEW:
                ...
                X_ft   = td["data"][idx_ft]
                amr_ft = td["amr"][idx_ft]
                X_test   = td["data"][idx_test]
                amr_test = td["amr"][idx_test]
...
csv_path = OUTPUT_PATH / f"amr_finetuning_{SPECIES}_MSUMG_all_{timestamp}.csv"
...
############################################################
print(f"\n{'='*70}")
print("SUMMARY (mean ± std across partitions, all MS-UMG years)")
print(f"{'='*70}")
...
if len(df_results) > 0:
    ...
    for dom in TARGET_DOMAINS:
        sub_dom = df_results[df_results["target_domain"] == dom]
        if len(sub_dom) == 0:
            continue
        print(f"\n{'='*60}")
        print(f"  {dom}")
        ...
        dom_tag = dom.replace("-", "").replace("_", "")
        summary.to_csv(OUTPUT_PATH / f"summary_global_{dom_tag}_{timestamp}.csv", index=False)
        ...
        for atb in ab_list:
            sub_atb = sub_dom[sub_dom["antibiotic"] == atb]
            if len(sub_atb) == 0:
                continue
            print(f"\n--- {atb} ---")
            per_partition_atb = (
            ...
            atb_summary.to_csv(OUTPUT_PATH / f"summary_{atb}_{dom_tag}_{timestamp}.csv", index=False)
