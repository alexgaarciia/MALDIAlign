...
print("\nRunning Global GridSearch (Random Forest)...")
...
grid_orig.fit(X_all_train, y_all_train)

rf_master_orig = grid_orig.best_estimator_

print("Best params (orig):", grid_orig.best_params_)
print("Best CV score (orig):", grid_orig.best_score_)

print("\n--- GridSearch LATENT ---")
grid_lat = GridSearchCV(
    rf_base,
    param_grid,
    cv=3,
    scoring="balanced_accuracy",
...
grid_lat.fit(Z_all_train, y_all_train)

rf_master_lat = grid_lat.best_estimator_
...
output_dir = Path("/export/usuarios01/agnavarr/MALDIAlign/experiments/finetuning/pretrained_rf") / timestamp
output_dir.mkdir(parents=True, exist_ok=True)

path_rf_orig = output_dir / "rf_original_ABC_MAR_RKI.joblib"
path_rf_lat  = output_dir / "rf_latent_ABC_MAR_RKI.joblib"

joblib.dump(rf_master_orig, path_rf_orig)
joblib.dump(rf_master_lat, path_rf_lat)

print("Saved:", path_rf_orig)
print("Saved:", path_rf_lat)
