...
for k, sk in SPLIT_NAMES.items():
    ...
    X_train_list.append(data_final[tr_idx]);  y_train_list.append(label_final[tr_idx])
    X_val_list.append(data_final[va_idx]);    y_val_list.append(label_final[va_idx])
    Z_train_list.append(Z_final[tr_idx]);     Z_val_list.append(Z_final[va_idx])
...
Z_train = np.vstack(Z_train_list);  Z_val   = np.concatenate(Z_val_list)

y_train_enc = le.transform(y_train)
y_val_enc   = le.transform(y_val)

# MLP on original space
...
for i_part in range(N_PARTITIONS):
    ...
    for n_prev in GRID_PREV:
        for n_new in GRID_NEW:
            ...
            idx_test = splits["DRIAMS_D"]["test"]
            idx_ft   = splits["DRIAMS_D"]["finetuning"]
            ...
            evals = [
                ...
                ("MLP_few",         metrics_report_mlp(test_loader_orig,   mlp_few_orig, "DRIAMS_D", device=device, class_names=le.classes_)),
                ("FT_full_MLP",     metrics_report_mlp(test_loader_full,   mlp_lat,      "DRIAMS_D", device=device, class_names=le.classes_)),
                ("FT_freeze_MLP",   metrics_report_mlp(test_loader_freeze, mlp_lat,      "DRIAMS_D", device=device, class_names=le.classes_)),
            ]
