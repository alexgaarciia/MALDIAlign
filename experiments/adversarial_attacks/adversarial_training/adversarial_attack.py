...
# ============================================================
print("\n===== LOADING DALMA =====")
vae = load_model(MultiVAE_Bernoulli_SpeciesPrior_Extended(input_dim=data_final.shape[1], latent_dim=64, num_domains=5, n_species=n_classes_pool), PRETRAINED_DALMA)
vae.to(device)
vae.eval()
...
mlp_raw = MLPClassifier_Extended(input_dim=X_tr_pool.shape[1], n_species=n_classes_pool, epochs=50, lr=1e-3, patience=10)
mlp_raw.trainloop(make_loader(X_tr_pool, y_tr_pool_enc, shuffle=True), make_loader(X_va_pool, y_va_pool_enc), device)

print("\n===== TRAINING LINEAR PROBE (ALL pooled) =====")
probe = LinearProbe_Extended(latent_dim=Z_tr_pool.shape[1], n_species=n_classes_pool, epochs=50, lr=1e-3, patience=10,)
probe.trainloop(make_loader(Z_tr_pool, y_tr_pool_enc, shuffle=True), make_loader(Z_va_pool, y_va_pool_enc), device)
...
df_adv = sweep_attacks(models_to_compare,data_msumg, y_msumg_enc, eps_list=EPSILON_LIST, pgd_steps_list=PGD_STEPS, device=device)
