...
mask_a       = (meta_driams["hospital"] == "DRIAMS_A").values
data_driams  = data_driams[mask_a]
label_driams = label_driams[mask_a]
amr_driams   = amr_driams[mask_a]
meta_driams  = meta_driams[mask_a].reset_index(drop=True)
print(f"DRIAMS-A: {len(data_driams)} samples")
...
def finetune(pretrained_model, X_ft, y_ft, seed):
    ...
    # 70/10 within trainval → 7/8 train, 1/8 val (≈ 87.5/12.5 of trainval = 70/10 of total)
    sss_inner = StratifiedShuffleSplit(n_splits=1, test_size=1/8, random_state=seed)
    tr_idx, vl_idx = next(sss_inner.split(X_trainval, y_trainval.astype(int)))
    ...
    # Deep copy so we don't modify the source-only model
...
for species, antibiotic in PAIRS:
    ...
    # ── DRIAMS-A train data ────────────────────────────────
    sp_mask_tr = (label_driams == species)
    ab_idx_tr  = get_ab_idx(antibiotic, ab_driams)
    if ab_idx_tr is None:
        print(f"  '{antibiotic}' not in DRIAMS. Skipping.")
        continue

    y_tr_raw = amr_driams[sp_mask_tr, ab_idx_tr]
    valid_tr = ~np.isnan(y_tr_raw)
    X_tr_src = data_driams[sp_mask_tr][valid_tr]
